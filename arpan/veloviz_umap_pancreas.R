...
sizes <- c(length(cells))
times <- matrix(nrow=0,ncol=3)
for (size in sizes) {
  # subsample to make things faster
  set.seed(0)
  good.cells <- sample(cells, size)
  ...
  a <- toc()
  times <- rbind(times, c("Veloviz", size, a$toc - a$tic))
  ...
  a <- toc()
  times <- rbind(times, c("UMAP", size, a$toc - a$tic))
  print(times)
